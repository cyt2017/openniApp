 ...
 )
{
    ...
    int pRngTmp = 0;
	{
        if((rngId == NULL)||(buffer==NULL)/*||\
        ((uint)rngId < 0x80000000)||((uint)buffer < 0x80000000)||\
        ((uint)rngId > 0x9fffffff)||((uint)buffer > 0x9fffffff)*/)
		{
            printf("rngBufGet err rngId%08X,buffer %08X\r\n",(uint)rngId,(uint)buffer);
            return -1;
		}
	}
 ...
 )
{
...

	{
        ...
        ((uint)rngId > 0x9fffffff)||((uint)buffer > 0x9fffffff)*/)
		{
            printf("rngBufPut err rngId%08X,buffer %08X\r\n",(uint)rngId,(uint)buffer);
            return -1;
		}
	}
